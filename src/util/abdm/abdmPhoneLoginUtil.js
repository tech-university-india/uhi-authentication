...
const publicKeyFetcher = require("./publicKeyUtil");
...
const generatePhoneLoginOTP = async (mobile) => {
  try {
    const response = await axios.post(
      ABDM_API_URLS.HEALTH_ID.LOGIN_GENERATE_OTP_MOBILE_URL,
      {
        mobile,
      }
    );
...
const verifyPhoneLoginOTP = async (txnId, otp) => {
  try {
    ...
    const response = await axios.post(
      ABDM_API_URLS.HEALTH_ID.LOGIN_VERIFY_OTP_MOBILE_URL,
      {
        otp: encryptedOtp,
        txnId,
      },
...
const resendPhoneLoginOTP = async (authMethod, txnId) => {
  try {
    const response = await axios.post(
      ABDM_API_URLS.HEALTH_ID.LOGIN_RESEND_OTP_MOBILE_URL,
      {
        authMethod,
        txnId,
      },
...
const getUserTokenByHealthId = async (healthId, txnId, authToken) => {
  try {
    const response = await axios.post(
      ABDM_API_URLS.HEALTH_ID.FETCH_USER_AUTHORIZED_TOKEN,
      {
        healthId,
        txnId,
      },
